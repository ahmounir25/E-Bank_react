...
const Profile = () => {
    ...
    const fetchUserProfile = async () => {
        ...
        try {
            ...
            if (response.data.StatusCode === 200) {
                ...
                setError(response.data.message || 'Error while fetching profile data');
            }
        } catch (error) {
            setError(error.response?.data?.message || error.message || 'Error while fetching profile data')
        } finally {
            setLoading(false);
        }
    }
    ...
    const uploadProfilePicture = async (file) => {
        ...
        try {
            const response = await apiService.uploadPicture(file);
            if (response.data.StatusCode === 200) {
                ...
                setError(response.data.message || 'Error while uploading picture')

            }

        } catch (error) {
            setError(error.response?.data?.message || error.message || 'Error while uploading picture')
        } finally {
            setUploading(false);
        }
    }
    ...
    const deletePic = async (url) => {
        ...
        setError('');
        try {
            const response = await apiService.deletePicture(url);
            if (response.data.StatusCode === 200) {
                ...
                setError(response.data.message || 'Error while deleting picture')
            }
        } catch (error) {
            setError(error.response?.data?.message || error.message || 'Error while deleting picture')
        } finally {
            setDeleting(false);
        }
    }
    ...
    if (!userData) {
        return (
            <div className="profile-container">
                <div className="error-message"> profile data is not available</div>
            </div>);
    }
                ...
                <div className="profile-picture-section">
                    ...
                    <div className="profile-picture-upload">
                        <div className="profile-picture">
                            <img
                                src={userData.profilePicUrl || "/profile-picture/default-img.png"}
                                alt=""
                            />
                        </div>
                        ...
                        <div className="upload-controls">
                            ...
                            {userData.profilePicUrl && (<button
                                onClick={() => deletePic(userData.profilePicUrl)}
                                ...
                                <div className="transactions-section">
                                    ...
                                    {account.transactions && account.transactions.length > 0 ? (
                                        <div className="transactions-list">
                                            {account.transactions?.slice().reverse().slice(0, 5).map(transaction => (
